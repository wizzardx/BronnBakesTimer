 ...
 * on the timer's completion status, obtained from [timerRepository]. Upon completion, the text color switches
 ...
 * @param timerRepository The repository ([ITimerRepository]) that offers the timer's completion status.
...
fun TotalTimeRemainingView(
    ...
    val isCompletedState by timerRepository.timerCompleted.collectAsState()
